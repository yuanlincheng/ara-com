...
public class RoleServiceImpl implements RoleService {
	...
	public void saveObj(Role obj, String operator, Object... values) throws ServiceException {
		//判别添加的角色是否已存在   0 标示增加
		if(!roleDao.checkExist(obj.getName(),0)){
			obj.setIsRoot("N");
			//保存角色对象
			roleDao.save(obj);
		}else{
	...
	@Override
	public void updateObj(Role obj, String operator, int id, Object... values) throws ServiceException{
		//判别修改的角色是否已存在
		if(!roleDao.checkExist(obj.getName(),id)){
			obj.setId(id);

			Role roleTemp = roleDao.findById(id);
			//role持久对象构造
			if(null != roleTemp){
	...
	@Override
	public List<Role> getObjListPage(PageVO page, Object... values) {
	...
	public void delObj(int id, Object... values) throws ServiceException {
        if(!managerDao.checkRoleExist(id)){
